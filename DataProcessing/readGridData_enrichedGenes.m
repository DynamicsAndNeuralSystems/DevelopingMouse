function readGridData_enrichedGenes(whatTimePointNow,thisCellType)
%%
...
energyGrids=cell(length(enrichedGenes.(thisCellType).ID),1);
...
steps=length(enrichedGenes.(thisCellType).ID);
%%
for j=1:length(enrichedGenes.(thisCellType).ID)
...
var_name1=strcat('energyGrids_',thisCellType,'_',timePoints{timePointIndex},'.mat');
