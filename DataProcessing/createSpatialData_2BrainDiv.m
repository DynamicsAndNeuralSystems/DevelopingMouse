...
numData=[587,1000,1000,1000,1000,1000,1000];
% user input: current time point
% timePointNow={'E11pt5'};
% user input: % of genes missing above which a voxel is filtered off
voxelThreshold=0.3;
...
for i=1:length(timePoints)
  ...
  for d=1:size(C,1)
      ...
      combNow1=C(d,1);
      combNow2=C(d,2);
      isBrainDivision1=ismember(annotationGrids{i},brainDivision.(brainDivisions{combNow1}).ID);
      isBrainDivision2=ismember(annotationGrids{i},brainDivision.(brainDivisions{combNow2}).ID);
      ...
      close(h)
      clear energyGridsNow energyGridsNow1 energyGridsNow2
      %% only keep good voxels
      % for matrix 1
      isGoodVoxel_1=(sum(isnan(voxGeneMat1),2) < voxelThreshold*size(voxGeneMat1,2));

      voxGeneMat1=voxGeneMat1(isGoodVoxel_1,:);

      % normalize
      voxGeneMat1=BF_NormalizeMatrix(voxGeneMat1,'scaledSigmoid');
      % for matrix 2
      isGoodVoxel_2=(sum(isnan(voxGeneMat2),2) < voxelThreshold*size(voxGeneMat2,2));

      voxGeneMat2=voxGeneMat2(isGoodVoxel_2,:);
      ...
      % only keep good voxels
      coOrds1=coOrds1(isGoodVoxel_1,:);
      coOrds2=coOrds2(isGoodVoxel_2,:);
      % Create distance matrix from only voxels selected for gene expression matrix
      [dataIndSelect1,~]=datasample([1:size(voxGeneMat1,1)],numData(i),'replace',false);
      ...
      [dataIndSelect2,~]=datasample([1:size(voxGeneMat2,1)],numData(i),'replace',false);
      ...
      geneCorr=zeros(numData(i),numData(i));
      % compute correlation coefficient
      h = waitbar(0,'Computing correlation coefficient matrix...');
      steps=size(geneCorr,1) ;
      for k=1:size(geneCorr,1) % row of voxGeneMat1
          for q=1:size(geneCorr,2) % row of voxGeneMat2
      ...
      distMat=ipdm(coOrds1(dataIndSelect1,:),coOrds2(dataIndSelect2,:))*resolutionGrid.(timePoints{i});
      %%
      % extract the correlation coefficients and distances
      corrCoeff=geneCorr(:);
      distances=distMat(:);
      spatialData_2brainDiv.(timePoints{i}).(brainDivisions{combNow1}).(brainDivisions{combNow2}).corrCoeff=corrCoeff;
      spatialData_2brainDiv.(timePoints{i}).(brainDivisions{combNow1}).(brainDivisions{combNow2}).distances=distances;
    ...
    % str = sprintf('Developing Mouse %s %s %s',timePoints{i},brainDivisions{combNow1},brainDivisions{combNow2});
    ...
    %
    % if whatNorm==' ';
    %     str=sprintf('Degree of freedom adjusted R-square, scaled sigmoid, %s %s',brainDivisions{combNow1},brainDivisions{combNow2});
    % else
    %     str=sprintf('Degree of freedom adjusted R-square, %s %s %s',whatNorm, brainDivisions{combNow1},brainDivisions{combNow2});
    % end
    % Title=title(str);
    ...
    % str=strcat('GeneCoexpress_',timePoints{i},brainDivisions{combNow1},brainDivisions{combNow2},'_random1000_annoOnly_30%filter_noSC_massiveVer.jpg');
    % saveas(f,str)
    % str=strcat('R-square_exp_voxel_',timePoints{i},brainDivisions{combNow1},brainDivisions{combNow2},'_random1000_annoOnly_30%filter_noSC_massiveVer.jpg');
    % saveas(g,str)
    % str=strcat('expoFit_voxel_',timePoints{i},brainDivisions{combNow1},brainDivisions{combNow2},'_random1000_annoOnly_30%filter_noSC_massiveVer.jpg');
    % saveas(w,str)
    % clear f
    % clear g
    % clear w
    % cd '/projects/kg98/Gladys/Output/makeGridData1_2BrainDiv_var'
    % str=strcat('makeGridData1_2BrainDiv',timePoints{i},brainDivisions{combNow1},brainDivisions{combNow2},'.mat');
    % save(str,'-v7.3')
  end
  clear energyGrids
end
...
toc
% save variables
str=fullfile('Matlab_variables','spatialData_2brainDiv.mat');
save(str,'spatialData_2brainDiv')
