...
for i=1:length(timePoints)
  ...
  isRightDir{i} = (angle_all{i}<=(1/4)|...
                ((angle_all{i}>=(3/4))&(angle_all{i}<=(5/4)))|...
                (angle_all{i}>=(7/4)));

  distances_all{i}=spatialData.distances_all(isRightDir{i});
  corrCoeff_all{i}=spatialData.corrCoeff_all(isRightDir{i});
end
