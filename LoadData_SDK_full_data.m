function [structures,structuresNeed,Exp,geneEntrez,geneList,timePoints] = LoadData_SDK_full_data(filterHow,whatNorm)
...
%% raw averaged
for i = 1:length(expMeasure)
    Exp.(expMeasure{i}).rawAverage = cell(numStructureNeed,1);
    for j = 1:8 % for each structure needed
        Exp.(expMeasure{i}).rawAverage{j}=Exp.(expMeasure{i}).raw{j};
    end
    Exp.(expMeasure{i}).rawAverage{9}=Exp.(expMeasure{i}).raw{35};
    for j=10:11
        Exp.(expMeasure{i}).rawAverage{j}=Exp.(expMeasure{i}).raw{(j+1)};
    end
    Exp.(expMeasure{i}).rawAverage{12}=Exp.(expMeasure{i}).raw{36};
    for j=13:31
        Exp.(expMeasure{i}).rawAverage{j}=Exp.(expMeasure{i}).raw{(j+3)};
    end
end
