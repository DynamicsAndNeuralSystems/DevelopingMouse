...
%% [Binning] Plotting each time point as one colour, other data points black
% first, get the colours needed
cmapOut = BF_getcmap('spectral',7,0,0);
...
for k=1:7
    ...
    for i=1:7
        ...
        if i~=k
            plotColor='k';
            BF_PlotQuantiles_diffColor(distance_clean,corrCoeff_clean,20,0,plotColor,0); % plot immediately
            hold on
        ...
        % plot coloured own dots
        
        if i~=k
            dataColor='k';
            s=scatter(distance_clean,corrCoeff_clean,nodeSize,dataColor,'LineWidth',0.3); % plot immediately
            set(s,'Marker','.')
            ylim([-0.4 0.8])
...
%% [Exponential fit] Plotting each time point as one colour, other data points black
% first, get the colours needed
cmapOut = BF_getcmap('spectral',7,0,0);
%% second, plot data from different time points in quantiles in the same graph 
% plot coexpression against distance for each of the 7 time points, and fitting
...
%b=figure('color','w');
for i=7:-1:1 % plot P28 first so that it does not cover other dots
    slice=squeeze(gene3D(i,:,:))'; % makes a matrix of 78 (structure) x 2100 (genes)
    % filter off structures with more than 10% of genes missing
    isMissing=(sum(~isnan(slice),2) <= 0.1*length(geneList));
    slice_clean=slice(~isMissing,:);
...
%%
    % fitting (separated from above for now)
    for j=1:length(fitMethods)
