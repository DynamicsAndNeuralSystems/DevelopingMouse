...
for i = 1:length(expMeasure)
    ...
    for j = 1:numStructures
            ...
            Exp.(expMeasure{i}).raw{j} = importfile(fileName);
            %Exp.(expMeasure{i}).raw{j}=cell2mat(textscan(fopen(fileName),'%f',2104,'delimiter','\n'));
            %Exp.(expMeasure{i}).raw{j}=xlsread(fileName,1,'A1:CBX7');
            %fclose('all');
    end
end
...
for g = 1:length(expMeasure)
    ...
    for i = 1:numStructures
        Exp.(expMeasure{g}).norm{i} = BF_NormalizeMatrix(Exp.(expMeasure{g}).raw{i},'scaledSigmoid');
    end
